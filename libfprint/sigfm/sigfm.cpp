...
template <> struct serializer<SigfmImgInfo> : public std::true_type
{
  static void
  serialize (const SigfmImgInfo &info, stream &out)
  {
    out << info.keypoints << info.descriptors;
  }
};

template <> struct deserializer<SigfmImgInfo> : public std::true_type
{
  static SigfmImgInfo
...
// best ar*(1-far)^3 v2
constexpr auto distance_match = 0.85;
constexpr auto length_match = 0.0275;
constexpr auto angle_match = 0.08;
constexpr auto min_match = 5;
...
struct match
{
  ...
  bool
  operator== (const match &right) const
  {
...
int
sigfm_fp_qualified (const SigfmPix *pix, int width, int height)
{
  try
    {
      cv::Mat img;
      ...
      // 2. Gather all contour points
      std::vector<cv::Point> allPts;
      for (auto &cnt : contours)
        {
          // if this contour area is < 10 px^2, skip it
          if (cv::contourArea (cnt) < 10)
            continue;
          allPts.insert (allPts.end (), cnt.begin (), cnt.end ());
        }
      ...
      // 4. Compare hull area to image area
      double hullArea = cv::contourArea (hull);
      double imgArea = static_cast<double> (width) * height;

      if ((hullArea / imgArea) < 0.8) {
        return false;
      }

      // 3. Border stripe detection
      const int stripeLen = 20, stripeTh = 4;
      // Horizontal stripes
      for (int x = 0; x <= width - stripeLen; ++x)
        {
          cv::Rect topROI (x, 0, stripeLen, stripeTh),
            botROI (x, height - stripeTh, stripeLen, stripeTh);
          if (cv::countNonZero (img (topROI)) == 0 ||
              cv::countNonZero (img (botROI)) == 0)
            return false;
        }
      // Vertical stripes
      for (int y = 0; y <= height - stripeLen; ++y)
        {
          cv::Rect leftROI (0, y, stripeTh, stripeLen),
            rightROI (width - stripeTh, y, stripeTh, stripeLen);
          if (cv::countNonZero (img (leftROI)) == 0 ||
              cv::countNonZero (img (rightROI)) == 0)
            return false;
        }
      return true;
  } catch (...)
  ...
  try
    {
      ...
      cv::Mat descs;
      cv::SIFT::create (0, 3, 0.01, 10, 1.6)->detectAndCompute (img, roi, pts, descs);
      ...
      for (const auto &pts : points)
        {
          ...
          if (match_1.distance < distance_match * pts.at (1).distance)
            {
              matches_unique.emplace (match{ frame->keypoints.at (match_1.queryIdx).pt,
          ...
          for (std::size_t k = j + 1; k < matches.size (); k++)
            {
              ...
              int vec_1[2] = { match_1.p1.x - match_2.p1.x, match_1.p1.y - match_2.p1.y };
              int vec_2[2] = { match_1.p2.x - match_2.p2.x, match_1.p2.y - match_2.p2.y };

              double length_1 = sqrt (pow (vec_1[0], 2) + pow (vec_1[1], 2));
              double length_2 = sqrt (pow (vec_2[0], 2) + pow (vec_2[1], 2));

              if (1 - std::min (length_1, length_2) / std::max (length_1, length_2) <= length_match)
                {

                  double product = length_1 * length_2;
                  angles.emplace_back (
                    angle (M_PI / 2 + asin ((vec_1[0] * vec_2[0] + vec_1[1] * vec_2[1]) / product),
          ...
          for (std::size_t k = j + 1; k < angles.size (); k++)
            {
              ...
              if (1 - std::min (angle_1.sin, angle_2.sin) / std::max (angle_1.sin, angle_2.sin) <=
                    angle_match &&
                  1 - std::min (angle_1.cos, angle_2.cos) / std::max (angle_1.cos, angle_2.cos) <=
                    angle_match)
                {
